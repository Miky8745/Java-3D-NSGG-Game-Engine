...
public class ObjectLoader {
    ...
    public Model loadModel(float[] vertices,float[] textureCoordinates, float[] normals, int[] indices, String name) {
        int id = createVAO();
        storeIndicesBuffer(indices);
        storeDataInAttribList(0,3,vertices);
        storeDataInAttribList(1,2,textureCoordinates);
        storeDataInAttribList(2,3,normals);
        unbind();
        return new Model(id, indices.length, name);
    }
    ...
    public int loadTexture(String filename) throws Exception {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (buffer == null) {
                throw new Exception("Image File " + filename + " did not load " + STBImage.stbi_failure_reason());
            }
    ...
    public Model loadOBJModel(String filename) {
        ...
        return loadModel(verticesArr, texCoordArr, normalArr, indicesArr, name);
    }
